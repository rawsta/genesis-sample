...
add_filter( 'style_loader_tag', 'raw_child_remove_type_attr', 10, 2 );
add_filter( 'script_loader_tag', 'raw_child_remove_type_attr', 10, 2 );
/**
 * Remove obsolete 'type=...' from script/style tags.
 *
 * @param string $tag All script tags.
 */
function raw_child_remove_type_attr( $tag, $handle ) {
	return preg_replace( '/ type=[\'"]text\/(javascript|css)[\'"]/', '', $tag );
}

add_filter( 'script_loader_tag', 'raw_child_js_defer_attr', 10 );
/**
 * Function to add defer to all scripts.
 *
 * @param string $tag All script tags.
 */
function raw_child_js_defer_attr( $tag ) {
