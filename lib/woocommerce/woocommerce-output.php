 ...
 * Raw Child.
 *
 * This file adds the WooCommerce styles and the Customizer additions for WooCommerce to the Raw Child Theme.
 *
 * @package Raw Child
 * @author  rawsta
 ...
 */
function raw_child_woocommerce_styles( $enqueue_styles ) {

	$enqueue_styles[ genesis_get_theme_handle() . '-woocommerce-styles' ] = [
		'src'     => get_stylesheet_directory_uri() . '/lib/woocommerce/raw-child-woocommerce.css',
		'deps'    => '',
...
add_action( 'wp_enqueue_scripts', 'raw_child_woocommerce_css' );
/**
 ...
 */
function raw_child_woocommerce_css() {
	...
	$color_link   = get_theme_mod( 'raw_child_link_color', $appearance['default-colors']['link'] );
	$color_accent = get_theme_mod( 'raw_child_accent_color', $appearance['default-colors']['accent'] );
	...
	',
		$color_accent,
		raw_child_color_contrast( $color_accent )
	) : '';
